...
def write_last_log_to_csv(source_log, output):
    with open(source_log) as f:
        ...
        diff_list = result_list.copy()
        for item in result_list:
            for row in diff_list:
                if item['Email'] == row['Email']:
                    item_date = convert_str_to_datetime(item['Last Changed'])
                    row_date = convert_str_to_datetime(row['Last Changed'])
    ...
    with open(output, 'w', newline="") as f: #newline="" необходимо вставлять только для Windows
        writer = csv.DictWriter(f, fieldnames=list(result_list[0].keys()))
        writer.writeheader()
