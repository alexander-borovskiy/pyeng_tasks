...
def create_vpn_config(template1, template2, data_dict):
    template_path1, template_file1 = os.path.split(template1)
    template_path2, template_file2 = os.path.split(template2)
    env = Environment(
                      loader=FileSystemLoader(template_path1),
                      trim_blocks=True,
                      lstrip_blocks=True
                      )
    templ1 = env.get_template(template_file1)
    templ2 = env.get_template(template_file2)
